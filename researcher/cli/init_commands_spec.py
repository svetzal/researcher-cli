...
from researcher.exceptions import ConfigurationError, StorageError
...
class DescribeInitCommand:
    ...
    def should_render_sibling_domain_error(self):
        mock_service = Mock(spec=SkillInstallService)
        mock_service.install.side_effect = ConfigurationError("Failed to load config file 'x': bad yaml")
