...
class ChromaGateway:
    @wrap_storage_error("Failed to open ChromaDB store at {persist_directory}: {e}")
    ...
    @wrap_storage_error("Failed to add fragments with embeddings to '{collection_name}': {e}")
    ...
    @wrap_storage_error("Failed to query collection '{collection_name}' with embedding: {e}")
    ...
    @wrap_storage_error("Failed to delete fragments for '{document_path}': {e}")
    ...
    @wrap_storage_error("Failed to delete collection '{collection_name}': {e}")
    def delete_collection(self, collection_name: str) -> None:
        self._client.delete_collection(name=collection_name)

    @wrap_storage_error("Failed to count fragments in '{collection_name}': {e}")
    def count(self, collection_name: str) -> int:
        collection = self._collection(collection_name)
        return collection.count()

    @wrap_storage_error("Failed to retrieve metadata batch from '{collection_name}': {e}")
